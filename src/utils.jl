...
function contrast(rbe::RBE, L::Matrix; numdf = 0, name = "Contrast", memopt = true)::ContrastTable
    ...
    if rank(L) ≥ 2
        ...
        for i = 1:length(vm)
            g       = ForwardDiff.gradient(x -> lclgf(L[i:i,:], L[i:i,:]', rbe.Xv, rbe.Zv, varlink(x); memopt = memopt), collect(rbe.θ))
            df      = 2*((L[i:i,:]*rbe.C*L[i:i,:]')[1])^2/(g'*(rbe.A)*g)
            vm[i]   = df/(df-2)
        end
        df = 2*sum(vm)/(sum(vm)-rank(L))
    else
        g       = ForwardDiff.gradient(x -> lclgf(L, L', rbe.Xv, rbe.Zv, varlink(x); memopt = memopt), collect(rbe.θ))
...
function estimate(rbe::RBE, L::Matrix; df = :sat, name = "Estimate", memopt = true, alpha = 0.05)
    ...
    if df == :sat
        θ       = theta(rbe)
        g       = ForwardDiff.gradient(x -> lclgf(L, L', rbe.Xv, rbe.Zv, varlink(x); memopt = memopt), θ)
        df      = 2*((lcl)[1])^2/(g'*(rbe.A)*g)
    elseif df == :cont
        df      = rbe.design.df3
    else
